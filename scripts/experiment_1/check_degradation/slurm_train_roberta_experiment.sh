...
#training_methods=("full_finetuning")
...
for i in ${!training_methods[@]}; do
    for j in ${!datasets[@]}; do
        ...
        print_current_time
        python src/train_detector.py --batch_size=$batch_size --num_epochs=1 --log_mode=online --freeze_base=$freeze_base --use_adapter=$use_adapter --save_dir=./saved_training_logs_experiment_1 --dataset_path=./fake_true_datasets/fake_true_dataset_${datasets[$j]}_10k --detector=$detector --eval_steps=$eval_steps --wandb_experiment_name=detector_training_experiment1 --check_degradation=500 --learning_rate=$learning_rate
    done
